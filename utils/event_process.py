...
def events_to_voxel_grid(events, num_bins, width, height, is_reverse=False):
    """
    ...
    t_norm = (num_bins - 1) * (events[:, 0] - first_stamp) / deltaT
    ts = t_norm
...
def events_to_voxel_grid_pol(events, num_bins, width, height):
    """
    Build a voxel grid with bilinear interpolation in the time domain from a set of events.
    :param events: a [N x 4] NumPy array containing one event per row in the form: [timestamp, x, y, polarity]
    :param num_bins: number of bins in the temporal axis of the voxel grid
    ...
    voxel_grid = np.zeros((num_bins, 2, height, width), np.float32).ravel()
    ...
    ys = events[:, 2].astype(np.uint)
    pols = events[:, 3]
    ps = pols.astype(np.uint) # 0,1
    pols[pols == 0] = 1. # values are 1
    ...
    valid_indices = tis < num_bins
    np.add.at(voxel_grid, xs[valid_indices] + ys[valid_indices] * width +
             ps[valid_indices]* width * height + tis[valid_indices] * width * height * 2, vals_left[valid_indices])

    valid_indices = (tis + 1) < num_bins
    np.add.at(voxel_grid, xs[valid_indices] + ys[valid_indices] * width
              + ps[valid_indices]* width * height + (tis[valid_indices] + 1) * width * height*2, vals_right[valid_indices])

    voxel_grid = np.reshape(voxel_grid, (num_bins, 2, height, width))
...
def event_preprocess_pytorch(event_voxel_grid, mode='std', filter_hot_pixel=False):
# Normalize the event tensor (voxel grid) so that
# the mean and stddev of the nonzero values in the tensor are equal to (0.0, 1.0)
    num_bins = event_voxel_grid.shape[0]
    if filter_hot_pixel:
